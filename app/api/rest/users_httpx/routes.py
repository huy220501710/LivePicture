...
async def get_users():
    ...
    if response.status_code != 200:
        raise HTTPException(
            status_code=response.status_code, detail="Error fetching users"
        )
...
async def create_user(user_data: dict):
    ...
    if response.status_code != 201:
        raise HTTPException(
            status_code=response.status_code, detail="Error creating user"
        )
    return response.json()
...
async def update_user(user_id: int, user_data: dict):
    ...
    if response.status_code not in {200, 204}:
        raise HTTPException(
            status_code=response.status_code, detail="Error updating user"
        )
    return response.json()
...
async def delete_user(user_id: int):
    ...
    if response.status_code != 200:
        raise HTTPException(
            status_code=response.status_code, detail="Error deleting user"
        )
